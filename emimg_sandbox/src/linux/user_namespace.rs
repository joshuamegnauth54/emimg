...
use crate::{
    ...
    linux::{DIR_FLAGS, RESOLVE_BENEATH_FLAGS, mount_namespace::private_recursive_mount},
...
pub unsafe fn enter_user_namespace() -> Result<SandboxClone, SandboxError> {
    ...
    // Impede mount propagation in child
    // https://lwn.net/Articles/689856/
    private_recursive_mount(
        Stage::CloneNamespace,
        "MS_PRIVATE in new user namespace (PID 1)",
    )?;

    child_wait_namespace(events)
}
...
fn parent_write_id_map(child: Pid, events: OwnedFd) -> Result<(), SandboxError> {
    ...
    let proc_dir = openat2(
        proc_dir,
        scratch.as_c_str().ok_or(SandboxError::buf_full(
            Stage::WriteIdMap,
            "C string'd child PID",
        ))?,
        DIR_FLAGS,
        Mode::empty(),
...
// Wait on parent to complete UID/GID maps.
fn child_wait_namespace(events: OwnedFd) -> Result<SandboxClone, SandboxError> {
